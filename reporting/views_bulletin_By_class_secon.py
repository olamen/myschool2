...
def generate_class_report_pdf(request, sessionyear_id, class_id):
    ...
    students = Student.objects.filter(studentclass_id=class_id).order_by("first_name")
    subjects = Subject.objects.filter(grade=student.student_class_id.grade, is_active=True)  # Moved outside student loop
